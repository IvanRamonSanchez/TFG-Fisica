...
class Fluid:
    def __init__(self, numX, numY, numZ, h):
        ...
        self.Y = numY 
        self.Z = numZ 
        self.x = np.arange(numX)
        self.y = np.arange(numY)
        self.z = np.arange(numZ)
        ...
        self.estado = malla_CAMPILLO #np.ones([self.X, self.Y, self.Z])

        self.x_fluido, self.y_fluido, self.z_fluido = np.where(self.estado[:, :, :] == 1)
    ...
    def flujo(self):
        self.vel[self.x_fluido, self.y_fluido, self.z_fluido, 0] = param['flujo']
        ...
        for i in range(self.X):
            for j in range(self.Y):
                for k in range(self.Z-1):
                    if self.estado[i,j,k] != 0 and self.estado[i, j, k-1] != 0:
    ...
    def simulation(self):
        ...
        tiempos = os.path.join("C:/Users/ramon/Desktop/SIMULACIONES_TFG_FÍSICA/Campillo EF3D", 'Tiempos de simulación (EF3D).txt')
        if self.n == 1:
            open(tiempos, 'w')
...
z_planta = 6
x_obs_planta, y_obs_planta = np.where(f.estado[:, :, z_planta] == 0)

y_alzado = 100
x_obs_alzado, z_obs_alzado = np.where(f.estado[:, y_alzado, :] == 0)
...
def main():   
    for iter in tqdm(range(param['frames'])):
        ...
        if iter % param['plotFrec'] == 0 and iter >= param['skip_first_frames']:
            ...
            #Plot planta
            presion_planta = pressure[:, :, z_planta].T
            velocidad_planta = velocidad[:, :, z_planta, :]
            ...
            for i in range(1, f.X, 8):
                for j in range(1, f.Y, 8):
                    if f.estado[i, j, z_planta] == 1 and f.estado[i-1, j, z_planta] == 1 and i < f.X - 1:
                        x, y, z = get_streamlines(i, j, z_planta)
                        y_1 = [y_aux / param['h'] for y_aux in y]
                        x_1 = [x_aux / param['h'] for x_aux in x]
                        axs[0].plot(x_1, y_1, color= 'black', linewidth = 0.5)

            #Plot alzado
            presion_alzado = pressure[:, y_alzado, :].T
            velocidad_alzado = velocidad[:, y_alzado, :, :]
            ...
            for i in range(1, f.X, 8):
                for k in range(1, f.Z, 8):
                    if f.estado[i, y_alzado, k] == 1 and f.estado[i-1,  y_alzado, k] == 1 and i < f.X - 1:
                        x, y, z = get_streamlines(i, y_alzado, k)
                        x_2 = [x_aux / param['h'] for x_aux in x]
                        z_2 = [z_aux / param['h'] for z_aux in z]
                        axs[1].plot(x_2, z_2, color= 'black', linewidth = 0.5)
            ...
            frame_name = os.path.join("C:/Users/ramon/Desktop/SIMULACIONES_TFG_FÍSICA/Campillo EF3D", 'Frame_' + str(frame_iter))
