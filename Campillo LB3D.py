...
def main():
    def update(discrete_vel_0):
        ...
        macro_vel_0[0:5, :, :, :] = vel_profile[0:5, :, :, :]
        density_0[0:5, :, :] = (get_density(discrete_vel_0[0:5, :, :, x_0_vel]) + 2 * get_density(discrete_vel_0[0:5, :, :, x_neg_vel])) / (1 - macro_vel_0[0:5, :, :, 0])

        #(4) f_eq 
        f_eq = get_f_eq(macro_vel_0, density_0)

        #(3) 
        discrete_vel_0[0:5, :, :, x_pos_vel] = f_eq[0:5, :, :, x_pos_vel]
    ...
    for iter in tqdm(range(n_iter)):
        ...
        tiempos = os.path.join("C:/Users/ramon/Desktop/SIMULACIONES_TFG_FÍSICA/Campillo LB3D", 'Tiempos de simulación (LB3D).txt')
        if n == 0:
            open(tiempos, 'w')
        ...
        if iter % plot_n_steps == 0  and iter >= skip_first_iter:
            ...
            fig.suptitle('Método Lattice Boltzmann 3D (t = ' + str(t_iter) + ' min)', fontsize= 20)
    
            #Plot planta
            vel_magnitude_planta = vel_magnitude[:, :, z_planta].T

            im1 = axs[0].imshow(vel_magnitude_planta, origin= 'lower', cmap= 'rainbow', vmin= 0, vmax= 1.5*inflow_vel)
            ...
            im2 = axs[1].imshow(vel_magnitude_alzado, origin= 'lower', cmap= 'rainbow', vmin= 0, vmax= 1.5*inflow_vel)
            ...
            frame_name = os.path.join("C:/Users/ramon/Desktop/SIMULACIONES_TFG_FÍSICA/Campillo LB3D", 'Frame_' + str(frame_iter))
